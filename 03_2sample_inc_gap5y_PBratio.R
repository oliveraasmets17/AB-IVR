...
# AB usage 2004-2014; disease incidence 2014-2022 (first five years taken as prevalent); exclude recent (6 months) AB users

# as sensitivity analysis, account for 5 years of incidence after follow-up as
# prevalent cases (i.e. exclude them from the sample)
...
#load the set of diagnoses, where first five years after start of follow up (2014) are counted as prevalent cases
diag_200k = readRDS("02_input/EstBB_diagnosis_200k_sensitivity_5y.rds")
prevotella_bacteroides = readRDS("02_input/metaphlan_PB_ratio.rds")

#take birthyear, gender and AB_n from the original diseases dataset
andmed_200k = andmed_200k %>% select(skood, birthyear, gender, AB_n)
#and merge with disease-information that is used for the sensitivity analysis (first five years of incidence
#excluded to account for feedback mechanism)
andmed_200k = merge(andmed_200k, diag_200k, by="skood")


####PREPROCESSING####
#exclude from EGC the infividuals in MB
...
for (j in 1:length(exposures)){
  ...
  #filtered list of prevalent diseases
  ...
  setwd("C:/Users/Administrator/Documents/01_projektid/09_AB_instrument/03_output/02_PBratio_gap5y/")
  write.xlsx(res_inc, paste(paste("res_inc_gap5y", exposure, min_cases, AB_max, min_age, max_age, sep="_"), ".xlsx", sep=""))
  
  ####PLOT THE RESULT####
  pdf(file = paste(paste("res_inc_gap5y", exposure, min_cases, AB_max, min_age, max_age, sep="_"), ".pdf", sep=""), 
      height=15, width=6)
